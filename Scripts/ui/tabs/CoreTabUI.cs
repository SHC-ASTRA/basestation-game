...
namespace UI
{
    public partial class CoreTabUI : BaseTabUI
    {
        ...
        private readonly Geometry.Vector3 Lin = new(), Ang = new();
        ...
        public void PushToPTZ()
        {
            ...
            ROS.Publish(CONTROL.PTZCONTROL, ptzMsg);
        ...
        public override void EmitToROS()
        {
            ...
            ROS.Publish(CONTROL.CORETWIST, twistMsg);

            controlMsg.max_duty = MaxSpeed;
            controlMsg.brake_mode = BrakeState;

            ROS.Publish(CONTROL.CORESTATE, controlMsg);
            ...
            if (WhiteButton != 0 || BlackButton != 0)
            {
                ptzMsg.zoom_level += WhiteButton - BlackButton;
                PushToPTZ();
            }
        }
        ...
        public override void STARTTAB()
        {
            ...
            ROS.AdvertiseTopic<CoreCtrlState>(CONTROL.CORESTATE, ControlQOS);
            ROS.AdvertiseTopic<Geometry.Twist>(CONTROL.CORETWIST, ControlQOS);
            ROS.AdvertiseTopic<PtzControl>(CONTROL.PTZCONTROL);

            const float deg2rad = Mathf.Pi / 180;
            const float rad2deg = 180 / Mathf.Pi;
            ROS.TopicSubscribe<NavSatFix>(FEEDBACK.COREGPS, (fix) =>
            {
            ...
            ROS.TopicSubscribe<Imu>(FEEDBACK.COREIMU, (data) =>
            {
        ...
        public override void STOPTAB()
        {
            ...
            ROS.Publish(CONTROL.CORETWIST, twistMsg);
            ROS.UnadvertiseTopic(CONTROL.CORETWIST);

            controlMsg.max_duty = 0f;
            controlMsg.brake_mode = true;
            ROS.Publish(CONTROL.CORESTATE, controlMsg);
            ROS.UnadvertiseTopic(CONTROL.CORESTATE);

            ROS.UnadvertiseTopic(CONTROL.PTZCONTROL);

            ROS.TopicUnsubscribe(FEEDBACK.COREGPS);
            ROS.TopicUnsubscribe(FEEDBACK.COREIMU);
        }
    }
}
